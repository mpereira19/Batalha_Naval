...
class BatalhaNavalWindow:
    ...
    def desenhaCasaAgua(self, coluna, linha):
        '''
        Desenha uma casa que esteja iluminada 
        :param coluna: indice da coluna 
        :param linha: indice da linha
        ...
        p2 = Point(p1.getX() + self.cell_size, p1.getY() + self.cell_size)
        r = Rectangle(p1, p2)
        r.setFill("#%02x%02x%02x" % (135, 206, 250))  
        r.draw(self.puzzle)
    ...
    def desenhaCasaBloqueadaNum_iluminada(self, coluna, linha, char):
        '''
        Desenha uma casa que esteja bloqueada e contenha um numero
        :param coluna: indice da coluna 
        :param linha: indice da linha
        ...
        label.setStyle("bold")
        label.setSize(24)
        r.setFill("#%02x%02x%02x" % (255, 128, 0))  # cor RGB da luz 
        label.draw(self.puzzle)     
    ...
    def desenhaletraLinha(self, linha):
        '''
    ...
    def mostraJanela(self, matriz):
        ...
        try:
            ...
            for line in matriz:
                for simbolo in line:
                    self.desenhaletraLinha(linha+1)
                    if simbolo == '.':
                        ...
                        self.desenhaCasaBloqueadaNum(coluna + 1, linha + 1, coluna)
                    coluna = coluna + 1
                coluna = 0
                linha = linha + 1
